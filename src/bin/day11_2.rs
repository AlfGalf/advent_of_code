...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let empty_cols: Vec<bool> = (0..chars[0].len())
        .map(|r| chars.iter().map(|v| v[r]).all(|c| c == '.'))
        .collect();

    let pos: Vec<(usize, usize)> = chars
        .iter()
        .enumerate()
        .flat_map(|(y, v)| {
            v.iter()
    ...
    let prod: usize = iproduct!(&pos, &pos)
        .map(|((fx, fy), (tx, ty))| {
            print!("({fx}, {fy}) ({tx}, {ty}), ");
            ...
            let res =
                fx.abs_diff(tx) + fy.abs_diff(ty) + num_cols * multiplier + num_rows * multiplier;
            println!("{}", res);
            res
