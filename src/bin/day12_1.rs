...
fn check(guess: &str, nums: &Vec<usize>) -> Option<bool> {
    ...
    if guess.contains('?') {
        if let Some(answered_section) = guess.split('?').next() {
            for (num, part) in answered_section.split('.').filter(|&x| !x.is_empty()).collect::<Vec<&str>>().into_iter().enumerate().rev().skip(1) {
                let Some(&n) = nums.get(num) else {return Some(false);};
                if part.len() != n { return Some(false); }
            }
            None
        } else {
            // No current guesses
    ...
    let res: usize = binding.lines().map(|l|{
        ...
        while let Some(guess) = guesses.pop() {
            match check(&guess, &nums) {
                ...
                Some(false) => {},
                None => {
                    let index = guess.find('?').expect("There should definitely be a ? if check is not certain");
