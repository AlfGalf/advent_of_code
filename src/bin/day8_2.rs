...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let instructions: HashMap<String, (String, String)> = instructions.captures_iter(&binding)
        .map(|c| (c.get(1).unwrap().as_str().to_string(), (c.get(2).unwrap().as_str().to_string(), c.get(3).unwrap().as_str().to_string()))).collect();

    let cur: Vec<&String> = instructions.keys().filter(|p| p.ends_with("A")).collect();

    let val: u128 = cur.into_iter().map(|s|{
        let mut i: u128 = 0;
        let mut cur = s;
        'label: loop {
            for char in first_line.chars() {
                // println!("{cur:?}");
                if cur.ends_with("Z") { break 'label; }
                i += 1;
                if char == 'L' {
