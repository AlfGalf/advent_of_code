...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let connected = |from_char: char, from_pos: (usize, usize), next_pos: (usize, usize)| -> bool {
        match (from_char, from_pos, next_pos) {
            ('|', (fx, fy), (nx, ny)) if (nx, ny) == (fx, fy + 1) || (nx, ny) == (fx, fy - 1) => {
                true
            }
            ('-', (fx, fy), (nx, ny)) if (nx, ny) == (fx + 1, fy) || (nx, ny) == (fx - 1, fy) => {
                true
            }
            ('7', (fx, fy), (nx, ny)) if (nx, ny) == (fx - 1, fy) || (nx, ny) == (fx, fy + 1) => {
                true
            }
            ('J', (fx, fy), (nx, ny)) if (nx, ny) == (fx - 1, fy) || (nx, ny) == (fx, fy - 1) => {
                true
            }
            ('L', (fx, fy), (nx, ny)) if (nx, ny) == (fx + 1, fy) || (nx, ny) == (fx, fy - 1) => {
                true
            }
            ('F', (fx, fy), (nx, ny)) if (nx, ny) == (fx + 1, fy) || (nx, ny) == (fx, fy + 1) => {
                true
            }
            ('S', _, _) => true,
            ('A', _, _) => true,
    ...
    while let Some(pos) = frontier.pop_front() {
        ...
        let Some(from_pipe) = pipes.get(pos.1).map(|v| v.get(pos.0)).flatten() else {
            continue;
        };
        for dir in &dirs {
            let next_pos = (
            ...
            // println!("{next_pos:?}");
            let Some(next_pipe) = pipes.get(next_pos.1).map(|v| v.get(next_pos.0)).flatten() else {
                continue;
            };
            // println!("{next_pipe:?}");
    ...
    for y in 0..pipes.len() {
        ...
        for x in 0..pipes[0].len() {
            ...
            if walls.contains(&(x, y)) {
                ...
                if pipe == 'J' && !from_above {
                    inside = !inside
                }
                if (pipe == '7' || pipe == 'S') && from_above {
                    inside = !inside
                }
